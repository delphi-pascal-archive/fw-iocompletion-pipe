...
type
  ...
  protected
    function CreatePipeInstance(out NewPipeHandle: THandle;
       Value: TOverlapped): Boolean;
    procedure DoWork;
  public
  ...
  TFWPipeData = record
    ...
    ReadBuff: array [0..MAXWORD - 1] of Char;
    ReadBuffSize: DWORD;
    WriteBuff: array [0..MAXWORD - 1] of Char;
    WriteBuffSize: DWORD;
    UserData: Pointer;
  end;

  TOnClientReadEvent = procedure(Sender: TObject;
    pData: Pointer; cbSize: DWORD) of object;
  ...
  private
    ...
    FBuff: array [0..MAXWORD - 1] of Char;
    function GetActive: Boolean;
    procedure SetActive(const Value: Boolean);
  public
    constructor Create(const ServerName, PipeName: string); virtual;
    destructor Destroy; override;
    procedure SendData(var pData: Pointer; var cbSize: DWORD); overload;
  ...
  procedure CompletedReadRoutine(dwErr, cbBytesRead: DWORD;
    lpOverLap: POverlapped); stdcall;
...
procedure CompletedWriteRoutine(dwErr, cbWritten: DWORD;
  lpOverLap: POverlapped); stdcall;
var
...
begin
  ...
  if (dwErr = 0) and (cbWritten = PipeInstance^.WriteBuffSize) then
    AResult := ReadFileEx(PipeInstance^.PipeHandle, @PipeInstance^.ReadBuff[0],
      MAXWORD, lpOverLap, @CompletedReadRoutine);
  if not AResult then
...
begin
  ...
  begin
    ...
    AResult := WriteFileEx(PipeInstance^.PipeHandle,
      @PipeInstance^.WriteBuff[0], PipeInstance^.WriteBuffSize,
...
function TFWPipeServer.CreatePipeInstance(out NewPipeHandle: THandle;
  Value: TOverlapped): Boolean;
const
  ...
  try
    ...
    try
      ...
      try
        ...
        NewPipeHandle :=
          CreateNamedPipe(PChar('\\.\pipe\' + FPipeName),
          PIPE_ACCESS_DUPLEX or FILE_FLAG_OVERLAPPED,
          PIPE_TYPE_MESSAGE or PIPE_READMODE_MESSAGE or PIPE_WAIT,
          PIPE_UNLIMITED_INSTANCES, MAXWORD, MAXWORD, NMPWAIT_USE_DEFAULT_WAIT,
          @Attributes);
  ...
  begin
    ...
    case Index of
      ...
      begin
        if PendingIO then
        begin
          if not GetOverlappedResult(NewPipeHandle,
            ConnectOverlapped, lpNumberOfBytesTransferred, False) then
            RaiseLastOSError
          else
            DoConnect(NewPipeHandle);
        end;
...
procedure TFWPipeClient.SendData(var pData: Pointer; var cbSize: DWORD);
begin
  Win32Check(TransactNamedPipe(FPipe, pData, cbSize,
    @FBuff[0], MAXWORD, cbSize, nil));
  pData := @FBuff[0];
end;
...
begin
  ...
  begin
    ...
    if FEvent = 0 then
      RaiseLastOSError;
    FPipe := CreateFile(PChar(FPipeName),
    ...
    if not WaitNamedPipe(PChar(FPipeName), NMPWAIT_WAIT_FOREVER) then
    begin
      LastError := GetLastError;
      Active := False;
      SetLastError(LastError);
